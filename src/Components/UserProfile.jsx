...
import { FaTrash, FaHistory, FaEllipsisV } from "react-icons/fa";
  ...
  return (
    <>
      <div className="mt-15 justify-center flex ">
        <h2 className="mt-6 text-white font-bold text-2xl">
          Welcome to your Profile{" "}
          <span className="capitalize text-3xl text-blue-200">
            ...
            {profilePosts.map((post) => (
              ...
              >
                ...
                <span
                  onClick={() =>
                    setActiveMenuPost(
                      activeMenuPost === post.id ? null : post.id
                    )
                  }
                ...
                {activeMenuPost === post.id && (
                  <div
                    ref={menuRef}
                    className="absolute right-2 top-8 w-32 bg-[#2b3542] border border-gray-600 rounded-xl shadow-2xl text-gray-200 z-50"
                  >
                    <button className="w-full text-left px-4 py-2 hover:bg-gray-700 hover:text-white rounded-t-xl transition-all duration-150">
                      Repost
                    </button>
                    <button className="w-full text-left px-4 py-2 hover:bg-red-700 hover:text-white rounded-b-xl transition-all duration-150">
                      Delete
                    </button>
                  </div>
                )}
      ...
      {userMenu && (
        <div
          ref={menuRef}
          className="absolute right-6 top-45 w-20 bg-white rounded-md shadow-lg p-2 text-gray-700"
        >
          <div className="">
            <button className="cursor-pointer hover:bg-gray-500">Repost</button>{" "}
            <br />
      ...
      {role === "helper" && (
        <div>
          {" "}
          <p className="text-xl text-white flex justify-center mt-10">
            We are Glad you stood out to volunteer for providing Help Support
          </p>
